...
export function buildContainerRunArgs(
  ...
  return [
    ...
    "NO_COLOR=1",
    "--mount",
    "type=bind,src=" + request.workspacePath + ",dst=" + CONTAINER_WORKSPACE_ROOT,
    "--mount",
    ...
    "codex",
    ...buildCodexArgs(request, config.codexSandboxMode, CONTAINER_WORKSPACE_ROOT),
  ];
}
...
export class ContainerCodexRunner implements AgentRunner {
  ...
  async run(request: RunnerRequest): Promise<RunnerResult> {
    ...
    request.emit({
      ...
      data: { containerName: name, image: this.config.containerRuntimeImage },
    });
    const child = spawn(
      this.config.containerEngine,
    ...
    request.emit({ stage: "container", severity: "success", message: "Container spawned", data: { containerName: name } });
    request.emit({ stage: "codex", message: "Codex process running inside container" });
    ...
    try {
      ...
      if (active.timedOut) {
        throw new RuntimeExitError("Runtime timed out after " + this.config.codexTimeoutMs + " ms", {
          exitCode: null,
      ...
      request.emit({
        ...
        data: { exitCode, containerName: active.containerName },
      });
      if (exitCode !== 0) {
        const detail = parsed.errors.at(-1) ?? stderr.trim() ?? "No error detail";
        throw new RuntimeExitError(
          this.config.containerEngine + " Runtime exited with code " + exitCode + ": " + detail,
          { exitCode, containerName: active.containerName, stdout, stderr, cause: new Error(detail) },
        );
      }
