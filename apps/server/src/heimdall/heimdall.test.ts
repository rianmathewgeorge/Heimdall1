...
describe("1 fail closed", () => {
  test("garbage, empty and invalid manifests all throw", () => {
    expect(() => parseManifest("I'm sorry, I can't do that.")).toThrow(ManifestError);
    expect(() => parseManifest('{"summary":"x"}')).toThrow(ManifestError);
    expect(() => parseManifest('{"summary":"x","capabilities":[]}')).toThrow(ManifestError);
    expect(() => parseManifest('{"summary":"x","capabilities":[{"op":"NUKE"}]}')).toThrow(ManifestError);
    expect(() => parseManifest('{"summary":"x","capabilities":[{"op":"NET_WRITE"}]}')).toThrow(ManifestError);
...
describe("manifest normalisation", () => {
  ...
  test("normalisation never rescues a manifest with nothing valid in it", () => {
    expect(() => parseManifest(m([{ op: "NET_READ" }, { op: "EXEC" }]))).toThrow(ManifestError);
