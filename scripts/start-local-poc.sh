...
proxy_image="${CONTAINER_PROXY_IMAGE:-heimdall-proxy:local}"
...
if [[ "${MODEL_PROVIDER:-ark}" == "openrouter" ]]; then
  if [[ -z "${OPENROUTER_API_KEY:-}" || -z "${OPENROUTER_MODEL:-}" ]]; then
    log "OPENROUTER_API_KEY and OPENROUTER_MODEL are required when MODEL_PROVIDER=openrouter."
    log "Example: MODEL_PROVIDER=openrouter OPENROUTER_API_KEY=sk-or-... OPENROUTER_MODEL=deepseek/deepseek-chat-v3-0324:free npm run poc"
    exit 1
  fi
...
heimdall_network="${HEIMDALL_NETWORK:-heimdall-internal}"
heimdall_egress_network="${HEIMDALL_EGRESS_NETWORK:-heimdall-egress}"
if [[ "${HEIMDALL:-on}" != "off" ]]; then
  if ! "$engine" network inspect "$heimdall_network" >/dev/null 2>&1; then
    log "Creating the isolated $heimdall_network network (one-time setup)."
    "$engine" network create --internal "$heimdall_network" >/dev/null
  fi
  # The proxy sidecar's only route to the internet. --internal networks have no
  # route back to the host at all (verified directly on Docker Desktop), so the
  # egress proxy can't run in-process anymore — it needs its own, real network.
  if ! "$engine" network inspect "$heimdall_egress_network" >/dev/null 2>&1; then
    log "Creating the $heimdall_egress_network network (one-time setup)."
    "$engine" network create "$heimdall_egress_network" >/dev/null
  fi
fi
...
if [[ "${HEIMDALL:-on}" != "off" ]]; then
  log "Building $proxy_image from Dockerfile.proxy (the egress proxy sidecar)."
  "$engine" build --file Dockerfile.proxy --tag "$proxy_image" .
fi
...
export CONTAINER_PROXY_IMAGE="$proxy_image"
export HEIMDALL_EGRESS_NETWORK="$heimdall_egress_network"
